...
public class MaskedImageLoadTask extends AsyncTask<Void, Void, Bitmap> {
	...
	public MaskedImageLoadTask(ImageTaskData taskData) {
		...
		this.maskImageKey = taskData.getMaskImageKey();
		this.maskDrawable = taskData.getMaskDrawable();
	}

	private String getCacheKey() {
		return ImageHelper.getCacheKey(taskData.getUrl(), maskImageKey);
	}
	...
	private Bitmap loadImage() {
		ImageTaskData newTaskData = new ImageTaskData(taskData.getUrl(), taskData.getSampleWidth(), null, null, false);
	...
	protected Bitmap doInBackground(Void... params) {
		...
		logger.d("doInBackground: %s %s", taskData.getUrl(), taskData.getMaskImageKey());
		...
		Bitmap maskImage = ImageHelper.convertDrawable(maskDrawable, bm.getWidth(), bm.getHeight());
		Bitmap maskedBitmap = ImageHelper.maskBitmap(bm, maskImage);
		
		if (maskedBitmap != null) {
			String cacheKey = getCacheKey();

			ImageCacheManager.putIntoFileCache(cacheKey, maskedBitmap);
	...
	protected void onPostExecute(final Bitmap result) {
		...
		ImageLoadManager.clearTask(taskData.getUrl(), taskData.getMaskImageKey());
