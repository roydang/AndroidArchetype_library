...
public class ImageCacheManager {
	...
	private synchronized static File newFile(String name) {
		...
		file = new File(BaseApplication._internalInstance.getExternalCacheFolder(), String.format("%s/%s", name.charAt(name.length()-1), name));
	...
	public static void putIntoFileCache(String url, Bitmap bitmap) {
		...
		ImageHelper.saveBitmap(bitmap, targetFile, format, 95);
	}
