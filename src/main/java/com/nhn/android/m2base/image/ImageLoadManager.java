...
public class ImageLoadManager implements Runnable {
	...
	public synchronized static void push(String url, boolean force, int sampleWidth, Object maskImageKey, Drawable maskDrawable, boolean downloadOnly, ApiRequestListener<Bitmap, ApiResponse> listener) {
		String key = ImageHelper.getCacheKey(url, maskImageKey);
		...
		if (instance.getTaskDataMap().containsKey(key)) {
			...
			if (force) {
				...
				if (taskData.getMaskDrawable() == null && taskData.getMaskImageKey() == null) {
		...
		taskData = new ImageTaskData(url, sampleWidth, maskImageKey, maskDrawable, downloadOnly);
