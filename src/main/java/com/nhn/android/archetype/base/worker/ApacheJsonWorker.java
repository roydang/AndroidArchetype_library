...
import com.nhn.android.archetype.base.worker.multipart.FilePart;
import com.nhn.android.archetype.base.worker.multipart.MultipartEntity;
import com.nhn.android.archetype.base.worker.multipart.Part;
import com.nhn.android.archetype.base.worker.multipart.StringPart;
...
public class ApacheJsonWorker extends MultipartJsonWorker {
	...
	protected void onSuccess(HttpResponse response, InputStream is, final FileCache cache) {
		if (jsonListener != null) {
			...
			if (cache != null) {
				if (cache.getJson().equals(json)) {
					...
					if (jsonListener instanceof PreloadJsonListener) {
						getHandler().post(new Runnable() {
							public void run() {
								((PreloadJsonListener) jsonListener).onSkipSuccess(cache.getModel());
							}
						});
					}
					return;
